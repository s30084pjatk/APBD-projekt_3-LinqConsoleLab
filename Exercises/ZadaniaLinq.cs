...
public sealed class ZadaniaLinq
{
    ...
    {
        return DaneUczelni.Przedmioty.Where(k => k.Kategoria == "Analytics").Take(1)
            .Select(s => $"{s.DataStartu:MM/dd/yyyy},{s.Nazwa}.").DefaultIfEmpty("Brak przedmiotu");
    ...
    {
        bool czyAktywny = DaneUczelni.Zapisy.Any(s => s.CzyAktywny = false);

        if (czyAktywny == false)
        {
            return new[] { "Są nieaktywne zapisy" };
        }
        else
        {
            return new[] { "Brak niekatywnych zapisów" };
        }
    }
    ...
    {
        bool czyBrakKatedry = DaneUczelni.Prowadzacy.Any(p => string.IsNullOrEmpty(p.Katedra));
        //throw Niezaimplementowano(nameof(Zadanie06_CzyWszyscyProwadzacyMajaKatedre));
        if (czyBrakKatedry)
        {
            return new[] { "nie wszystkie mają wprowadzone nazwy" };
        }
        else
        {
            return new[] { "wszystkie mają wprowadzone nazwy" };
        }
    }
    ...
    {
        int quantity = DaneUczelni.Zapisy.Count(s => s.CzyAktywny == true);
        return new[] { $"Ilość aktywnych zapisów to {quantity}" };
    }
    ...
    public IEnumerable<string> Zadanie09_TrzyNajnowszeZapisy()
    {
        ...
        return DaneUczelni.Zapisy.OrderByDescending(s => s.DataZapisu).Take(3)
            .Select(s => $"{s.DataZapisu:M/d/yy}, {s.StudentId}, {s.PrzedmiotId}");
    }
    ...
    public IEnumerable<string> Wyzwanie02_PrzedmiotyStartujaceWKwietniuBezOcenKoncowych()
    {
        ...
        return DaneUczelni.Przedmioty
            ...
            .Join(DaneUczelni.Zapisy, p => p.Id, z => z.PrzedmiotId, (przedmiot, zapis) => 
                new { przedmiot, zapis })
    ...
    public IEnumerable<string> Wyzwanie03_ProwadzacyISredniaOcenNaIchPrzedmiotach()
    {
        ...
        return DaneUczelni.Prowadzacy.SelectMany(p => DaneUczelni.Przedmioty.Where(pr => pr.ProwadzacyId == p.Id)
            .Select(pr => new { Prowadzacy = p, Przedmiot = pr }))
            .SelectMany(x => DaneUczelni.Zapisy.Where(z => z.PrzedmiotId == x.Przedmiot.Id)
                .Select(z => new { x.Prowadzacy, Zapis = z })).Where(x => x.Zapis.OcenaKoncowa != null)
            .GroupBy(x => new { x.Prowadzacy.Imie, x.Prowadzacy.Nazwisko })
            .Select(g => $"{g.Key.Imie} {g.Key.Nazwisko}, " +
                         $"{g.Average(x => x.Zapis.OcenaKoncowa.Value):F2}");
    }
    ...
    public IEnumerable<string> Wyzwanie04_MiastaILiczbaAktywnychZapisow()
    {
        ...
        return DaneUczelni.Studenci
            .Join(DaneUczelni.Zapisy, s => s.Id, z => z.StudentId, (s, z) => new { Miasto = s.Miasto, CzyAktywny = z.CzyAktywny })
            .Where(x => x.CzyAktywny).GroupBy(x => x.Miasto)
            .Select(g => new { Miasto = g.Key, Liczba = g.Count() })
            .OrderByDescending(x => x.Liczba).Select(x => $"{x.Miasto}, {x.Liczba}");
    }
